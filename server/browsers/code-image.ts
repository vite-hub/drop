...
${SYNTAX_CSS}
</style></head><body><div id="capture"><div class="window"><div class="toolbar"><div class="dots"><span class="dot"></span><span class="dot"></span><span class="dot"></span></div><span class="language">${language}</span></div><pre><code>${highlighted}</code></pre></div></div><script>const capture=document.querySelector("#capture");capture.dataset.captureWidth=String(capture.offsetWidth);capture.dataset.captureHeight=String(capture.offsetHeight)</script></body></html>`
}

function renderSvg(rendered: string) {
  const width = rendered.match(/data-capture-width="(\d+)"/)?.[1]
  const height = rendered.match(/data-capture-height="(\d+)"/)?.[1]
  if (!width || !height)
    throw new Error("[drop:code-image] Kitesurf did not measure the code image.")

  const document = rendered
    .replace(/^<html[^>]*><head>/, '<div xmlns="http://www.w3.org/1999/xhtml">')
    .replace(/<meta[^>]*>/, "")
    .replace("</head><body>", "")
    .replace(/<script>[\s\S]*?<\/script>/, "")
    .replace("</body></html>", "</div>")
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><foreignObject width="100%" height="100%">${document}</foreignObject></svg>`)
}
...
export default defineBrowser(async (input: CodeImageInput, { browser }) => {
  ...
  const html = renderDocument(input, highlighted)
  if ((input.format ?? "png") === "svg")
    return renderSvg(await browser.content({ html }))

  const response = await browser.run("screenshot", {
    html,
  ...
  if (!image.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])))
    throw new Error("[drop:code-image] Kitesurf returned an invalid PNG image.")
  return image
})
